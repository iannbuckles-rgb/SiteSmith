...
- **Live preview via an in-browser virtual server.** A scoped **service worker** serves each completed project revision from immutable `/preview/<id>/<revision>/…` URLs, so a frame never sees a half-written cache and the browser resolves every reference natively — relative and root-relative paths, **ES-module `import` / dynamic `import()`**, `fetch()`, `new URL(x, import.meta.url)`, web workers, and wasm — with correct `Content-Type` headers. Superseded builds are cancelled and their staging caches removed. When a project contains both a development `index.html` and browser-ready output such as `dist/index.html` or `build/index.html`, MockupSwap selects and isolates the built website so uncompiled TSX/Vue/Svelte sources cannot replace its assets or produce a blank canvas. Served HTML gets a tiny injected runtime (an in-memory storage shim, link-nav bridge, and runtime/resource error reporter). Preview build failures, compatibility limitations, missing resources, uncaught exceptions, and promise rejections appear above the canvas with a reload action. On first use the app runs a one-shot capability probe; browsers that can't run a worker-controlled iframe automatically fall back to the legacy in-iframe **blob pipeline** (flat HTML/CSS/images) with a visible compatibility notice.
...
- **Browser-local persistence** via IndexedDB: recover the active session after refresh, save named projects, and create named checkpoints. The top bar reports `Unsaved`, `Saving…`, `Saved`, or `Save at risk` from the active autosave generation, and the browser warns before unload whenever recovery data is not safely stored.
...
npm run test:e2e     # Playwright: large active preview, cancellation, export
